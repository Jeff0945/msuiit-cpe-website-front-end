                  ...
                  <div class="sm:col-span-8 lg:col-span-7">
                    ...
                    <section aria-labelledby="options-heading" class="mt-10">
                      ...
                      <form action="#" @submit.prevent="">
                            ...
                            <div class="grid grid-cols-4 gap-4">
                              <RadioGroupOption as="template" v-for="size in product.sizes" :key="size.name" :value="size" :disabled="!size.inStock" v-slot="{ active, checked }">
                                <div :class="[size.inStock ? 'cursor-pointer bg-white text-gray-900 shadow-sm' : 'cursor-not-allowed bg-gray-50 text-gray-200', active ? 'ring-2 ring-cpe-dark-blue-lighter' : '', 'group relative flex items-center justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1']">
                                  ...
                                  <span v-if="size.inStock" :class="[active ? 'border' : 'border-2', checked ? 'border-cpe-dark-blue-lighter' : 'border-transparent', 'pointer-events-none absolute -inset-px rounded-md']" aria-hidden="true" />
                        ...
                        <ButtonSpinner @click="load" :text="'Add to bag'" :loading="loading" type="submit" class="mt-6 flex w-full items-center justify-center rounded-md border border-transparent bg-cpe-dark-blue-lighter px-8 py-3 text-base font-medium text-white hover:bg-cpe-blue-gray focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"></ButtonSpinner>
                      </form>
                    </section>
                  </div>
                </div>
